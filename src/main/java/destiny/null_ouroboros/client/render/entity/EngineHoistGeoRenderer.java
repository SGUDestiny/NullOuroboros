...
public class EngineHoistGeoRenderer extends GeoEntityRenderer<EngineHoistEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, EngineHoistEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    private void renderEmissiveBone(PoseStack poseStack, EngineHoistEntity animatable, GeoBone bone, MultiBufferSource bufferSource, int packedOverlay) {
        ...
        if (bone.getName().endsWith("Emissive")) {
            ...
            boolean installed = partType == null || animatable.isPartInstalled(partType);
            if (installed) {
                Integer glowColor = null;
                if (partType != null) {
                    glowColor = animatable.getPartGlowColor(partType);
                ...
                RenderType glowType = RenderTypeRegistry.getEmissiveRenderType(DEFAULT);
                VertexConsumer glowBuffer = bufferSource.getBuffer(glowType);
                super.renderCubesOfBone(poseStack, bone, glowBuffer, LightTexture.FULL_BRIGHT, packedOverlay, r, g, b, 1f);
            }
        }
    ...
    public void renderRecursively(PoseStack poseStack, EngineHoistEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource,
        ...
        if (isEmissive) {
            for (GeoBone child : bone.getChildBones()) {
                this.renderRecursively(poseStack, animatable, child, renderType, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay,
        ...
        if (partType != null && animatable.isPartInstalled(partType)) {
            if (isEmissive) {
                ...
                if (partType == DusterbikePartType.KEY) glowColor = animatable.getPartMainColor(partType);
        ...
        for (GeoBone child : bone.getChildBones()) {
            this.renderRecursively(poseStack, animatable, child, boneRenderType, bufferSource, boneBuffer, isReRender, partialTick, packedLight, packedOverlay,
                    1f, 1f, 1f, alpha);
        }
        poseStack.popPose();
    }
